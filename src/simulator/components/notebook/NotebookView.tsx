...
import { tex, renderMarkdownToHtml } from "./notebookRender.ts";
...
function Markdown({ src }: { src: string }) {
  const html = useMemo(() => renderMarkdownToHtml(src), [src]);
...
function Output({ o }: { o: CellOutput }) {
  ...
  if (o.kind === "parameter") return <div className="text-xs text-stone-400">value = <span className="text-vermilion-300">{o.value}</span></div>;
  if (o.kind === "expression")
    return <div className="text-xs text-stone-400">resolved: <span className="font-mono text-stone-200">{o.printed}</span> · <span className="text-stone-500">{o.note}</span></div>;
  // analysis
  const r = o.result;
  return (
    <div className="space-y-1">
      <div className="text-xs text-stone-300">{r.identity}</div>
      {r.sections.slice(0, 3).map((s) => (
        <div key={s.title} className="rounded bg-white/[0.02] px-2 py-1 ring-1 ring-white/5">
          <div className="text-[10px] uppercase tracking-wide text-vermilion-300/60">{s.title}</div>
          {s.properties.slice(0, 4).map((p, i) => (
            <div key={i} className="flex justify-between gap-2 text-[11px]">
              ...
              <span className="text-right font-mono text-stone-200">{p.latex ? <span dangerouslySetInnerHTML={{ __html: tex(p.latex) }} /> : p.value}
...
const input = "rounded bg-stone-800/80 px-2 py-1 font-mono text-sm text-vermilion-100 outline-none focus:ring-1 focus:ring-vermilion-400";

function CellEditor({ cell, out }: { cell: Cell; out: CellOutput }) {
  const { updateCell, deleteCell, moveCell, duplicateCell } = useNotebook();
  return (
    <div className="rounded-lg border border-white/5 bg-[#1c1b18]">
      <div className="flex items-center gap-1 border-b border-white/5 px-2 py-1">
        <span className="mr-auto text-[10px] uppercase tracking-widest text-stone-500">{cell.kind}</span>
        <button onClick={() => moveCell(cell.id, -1)} className="px-1 text-stone-500 hover:text-vermilion-300" title="Up">↑</button>
        <button onClick={() => moveCell(cell.id, 1)} className="px-1 text-stone-500 hover:text-vermilion-300" title="Down">↓</button>
        <button onClick={() => duplicateCell(cell.id)} className="px-1 text-stone-500 hover:text-vermilion-300" title="Duplicate">⧉</button>
        <button onClick={() => deleteCell(cell.id)} className="px-1 text-stone-500 hover:text-red-300" title="Delete">×</button>
      </div>
      <div className="space-y-2 p-2">
        ...
        {cell.kind === "parameter" && (
          <>
            <div className="flex items-center gap-2 text-sm">
              <input className={`${input} w-16`} value={cell.name} onChange={(e) => updateCell(cell.id, { name: e.target.value })} />
              <span className="text-stone-500">=</span>
              <span className="w-16 text-right font-mono text-vermilion-200">{cell.value}</span>
        ...
        {cell.kind === "analysis" && (
          <>
            <div className="flex items-center gap-2 text-sm text-stone-400">inspect <input className={`${input} w-24`} value={cell.targetName} onChange={(e) => updateCell(cell.id, { targetName: e.target.value })} /></div>
            <Output o={out} />
          </>
        )}
      </div>
...
export function NotebookView() {
  ...
  const btn = "rounded px-2 py-1 text-xs text-stone-300 hover:bg-white/10 hover:text-white";
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex flex-wrap items-center gap-1 border-b border-white/5 bg-[#1c1b18] px-3 py-1.5">
        <input className="mr-2 rounded bg-transparent px-1 text-sm font-semibold text-stone-100 outline-none focus:bg-stone-800/60" value={s.exp.metadata.title} onChange={(e) => s.setTitle(e.target.value)} />
        ...
        <span className="mx-1 text-stone-700">|</span>
        <select className={`${btn} bg-stone-800/60`} value="" onChange={(e) => e.target.value && s.loadExample(e.target.value)}>
      ...
      <div className="flex min-h-0 flex-1">
        <main className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto max-w-3xl space-y-3 px-6 py-4">
            {s.exp.metadata.description && <p className="text-xs text-stone-500">{s.exp.metadata.description}</p>}
            {s.exp.cells.map((c) => <CellEditor key={c.id} cell={c} out={outputs[c.id]} />)}
            {!s.exp.cells.length && <div className="text-sm text-stone-500">Empty notebook — add a cell or load an example.</div>}
          </div>
        </main>
        {s.snapshots.length > 0 && (
          <aside className="w-48 shrink-0 overflow-y-auto border-l border-white/5 bg-[#1c1b18] p-2">
            <h3 className="mb-1 text-[10px] uppercase tracking-widest text-vermilion-300/70">Snapshots</h3>
            {s.snapshots.map((snap, i) => (
              <button key={i} onClick={() => s.restoreSnapshot(i)} className="block w-full rounded px-2 py-1 text-left text-xs text-stone-400 hover:bg-white/5 hover:text-vermilion-200">{snap.label}</button>
            ))}
          </aside>
        )}
      </div>
